...
# INKO 1D structure
class inko_1d_op(nn.Module):
    def __init__(self, op_size, modes_x = 16):
        super(inko_1d_op, self).__init__()
...
class inko_1d(nn.Module):
    def __init__(self, encoder, decoder, op_size, modes_x = 16, decompose = 4, linear_type = True, normalization = False):
        ...
        self.enc = encoder
        self.dec = decoder
        self.koopman_layer = inko_1d_op(self.op_size, modes_x = modes_x)
        self.w0 = nn.Conv1d(op_size, op_size, 1)
        self.linear_type = linear_type # If this variable is False, activate function will be worked after INKO Matrix
    ...
    def forward(self, x):
        ...
        for i in range(self.decompose):
            x1 = self.koopman_layer(x) # INKO Operator
            if self.linear_type:
                x = x + x1
            else:
                x = torch.tanh(x + x1)
...
# INKO 2D structure
class inko_2d_op(nn.Module):
    def __init__(self, op_size, modes_x, modes_y):
        super(inko_2d_op, self).__init__()
        self.op_size = op_size
        self.scale = (1 / (op_size * op_size))
        self.modes_x = modes_x
...
class inko_2d(nn.Module):
    def __init__(self, encoder, decoder, op_size, modes_x = 10, modes_y = 10, decompose = 6, linear_type = True, normalization = False):
        ...
        self.enc = encoder
        self.dec = decoder
        self.koopman_layer = inko_2d_op(self.op_size, self.modes_x, self.modes_y)
