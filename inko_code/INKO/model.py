from INKO.models import  inko_model
from INKO import utils
from INKO.models import inko_optimized
...
class INKO:
    def __init__(self, backbone = "inko_1d", autoencoder = "MLP", o = 16, m = 16, r = 8, t_in = 1, device = False):
    ...
    def compile(self):
        if self.autoencoder == "MLP":
            encoder = inko_model.encoder_mlp(self.t_in, self.operator_size)
            decoder = inko_model.decoder_mlp(self.t_in, self.operator_size)
            print("The autoencoder type is MLP.")
        elif self.autoencoder == "Conv1d":
            encoder = inko_model.encoder_conv1d(self.t_in, self.operator_size)
            decoder = inko_model.decoder_conv1d(self.t_in, self.operator_size)
            print("The autoencoder type is Conv1d.")
        elif self.autoencoder == "Conv2d":
            encoder = inko_model.encoder_conv2d(self.t_in, self.operator_size)
            decoder = inko_model.decoder_conv2d(self.t_in, self.operator_size)
...
#            print("The autoencoder type is MLP.")
            print("Wrong!")
        if self.backbone == "inko_1d":
            self.kernel = inko_model.inko_1d(encoder, decoder, self.operator_size, modes_x = self.modes, decompose = self.decompose).to(self.device)
            print("inko_1d model is completed.")
        
        elif self.backbone == "inko_2d":
            self.kernel = inko_model.inko_2d(encoder, decoder, self.operator_size, modes_x = self.modes, modes_y = self.modes,decompose = self.decompose).to(self.device)
            print("inko_2d model is completed.")

        if not self.kernel == False:
            self.params = utils.count_params(self.kernel)
            print("INKO Model has been compiled!")
    ...
    def save(self, path):
        ...
        if not os.path.isfile(fpath):
            os.makedirs(fpath)
        torch.save({"INKO":self,"model":self.kernel,"model_params":self.kernel.state_dict()}, path)

class INKO_optimized:
    def __init__(self, decoder = "Conv2d", depth = 16, resolution=(256, 256), patch_size=(4, 4),
        ...
        self.loss = torch.nn.MSELoss()
    def compile(self):
        self.kernel = inko_optimized.optimized_op(img_size=self.resolution, patch_size=self.patch_size, in_chans=self.in_chans, out_chans=self.out_chans, num_blocks=self.num_blocks, embed_dim = self.embed_dim, depth=self.depth, settings = self.decoder).to(self.device)
        if self.parallel:
            self.kernel = torch.nn.DataParallel(self.kernel)
        self.params = utils.count_params(self.kernel)
        
        print("INKO Fourier Vision Transformer has been compiled!")
        print("The Model Parameters Number is ",self.params)
